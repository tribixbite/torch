...
export type SerializedFilter =
	| { type: 'multi'; selected: string[]; mode: LogicMode }
	| { type: 'boolean'; yes: string[]; no: string[]; mode: 'all' | 'any' }
		...
		switch (filter.type) {
			case 'multi':
				if (filter.selected.size > 0) {
					result[col] = { type: 'multi', selected: [...filter.selected], mode: filter.mode };
				}
				break;
			case 'boolean':
				if (filter.yes.size > 0 || filter.no.size > 0) {
					result[col] = { type: 'boolean', yes: [...filter.yes], no: [...filter.no], mode: filter.mode };
				}
				break;
			case 'range':
				if (filter.minActive || filter.maxActive) {
					result[col] = { type: 'range', min: filter.min, max: filter.max };
				}
				break;
		}
	}
