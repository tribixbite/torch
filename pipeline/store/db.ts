...
export function getDb(): Database {
	if (!_db) {
		...
		_db.exec('PRAGMA busy_timeout = 30000'); // Wait up to 30s for concurrent writes
...
export function upsertFlashlight(entry: FlashlightEntry): void {
	...
	const stmt = db.prepare(`
		INSERT INTO flashlights (
			...
			runtime_hours = CASE WHEN excluded.runtime_hours != '[]' THEN excluded.runtime_hours ELSE runtime_hours END,
			measured_performance = excluded.measured_performance,
			battery = CASE WHEN excluded.battery != '[]' THEN excluded.battery ELSE battery END,
			wh = COALESCE(excluded.wh, wh),
			charging = CASE WHEN excluded.charging != '[]' THEN excluded.charging ELSE charging END,
			modes = CASE WHEN excluded.modes != '[]' THEN excluded.modes ELSE modes END,
			levels = COALESCE(excluded.levels, levels),
			blink = CASE WHEN excluded.blink != '[]' THEN excluded.blink ELSE blink END,
			...
			material = CASE WHEN excluded.material != '[]' THEN excluded.material ELSE material END,
			color = CASE WHEN excluded.color != '[]' THEN excluded.color ELSE color END,
			impact = CASE WHEN excluded.impact != '[]' THEN excluded.impact ELSE impact END,
			environment = CASE WHEN excluded.environment != '[]' THEN excluded.environment ELSE environment END,
			switch = CASE WHEN excluded.switch != '[]' THEN excluded.switch ELSE switch END,
			features = CASE WHEN excluded.features != '[]' THEN excluded.features ELSE features END,
			price_usd = COALESCE(excluded.price_usd, price_usd),
			purchase_urls = CASE WHEN excluded.purchase_urls != '[]' AND excluded.purchase_urls != '' THEN excluded.purchase_urls ELSE purchase_urls END,
			info_urls = CASE WHEN excluded.info_urls != '[]' AND excluded.info_urls != '' THEN excluded.info_urls ELSE info_urls END,
			image_urls = CASE WHEN excluded.image_urls != '[]' AND excluded.image_urls != '' THEN excluded.image_urls ELSE image_urls END,
